...
import (
	...
	"penishell/internal/common/struct/frame"
	"penishell/internal/common/type/create"
	"penishell/internal/common/type/env"
	"penishell/internal/common/type/list"
	"penishell/internal/common/type/obj"
	...
	"penishell/internal/engine/commands"
	"penishell/internal/system/cache"
...
// represented in penishell, when the condition in an if-statement is false, penishell has
...
func trace(t *T) Op {
	...
	for dup.stack != done {
		...
		if n != l {
			l = n
			if !strings.HasSuffix(l.Text, "# penishell:omit-from-trace") {
